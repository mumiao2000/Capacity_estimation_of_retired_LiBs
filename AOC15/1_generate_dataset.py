...
r = 0.6
window = int(r * 1392)  # 1392 is the average full length
...
def sampling(name_list, set_path, sampling_num, resolution):
    ...
    for file_name in tqdm.tqdm(name_list):
        ...
        key = int((df.iloc[0, 0] - 8.3075) / resolution) * resolution
        if key in dic.keys():
...
sampling(train_name_list, train_set_path, int(sampling_num * (1 - test_set_rate - valid_set_rate)), 4.190 / k)
sampling(valid_name_list, valid_set_path, int(sampling_num * valid_set_rate), 4.190 / k)
sampling(test_name_list, test_set_path, int(sampling_num * test_set_rate), 4.190 / k)